...
describe('inputTextWithMappedContextChipsFromPromptEditorState', () => {
    test('maps all expected mentions', async () => {
        const andNode = {
            detail: 0,
            format: 0,
            mode: 'normal',
        ...
        const editorState = editorStateFromPromptString(ps``)
        editorState.lexicalEditorState.root = {
            children: [
                {
                    children: [
                        {
        ...
        expect(inputTextWithMappedContextChipsFromPromptEditorState(editorState)).toBe(
            'What are @file and @repo and @dir and @symbol and @repo and @file and @dir'
        )
    })
})
