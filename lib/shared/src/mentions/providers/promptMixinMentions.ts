...
import { URL_CONTEXT_MENTION_PROVIDER } from './urlMentions'
...
export const PROMPT_MIXIN_MENTION_PROVIDER: ContextMentionProvider<'mixin'> = {
    ...
    async resolveContextItem(item, props, signal) {
        if (item.content !== undefined) {
            return [item as ContextItemWithContent]
        }

        if (item.type !== 'mixin' || item.uri.scheme !== 'cody') {
            //these are external prompts
            return []
        }

        const id = item.uri.path.split('/').at(-1) ?? ''
        const found = mixins.find(m => m.id === id)
        if (!found) {
            return []
        }
        const additionalUrlPromises =
            found.additionalUrls?.map(async ({ uri, title }) => {
                try {
                    return (
                        (await URL_CONTEXT_MENTION_PROVIDER.resolveContextItem?.(
                            { type: 'file', title, uri, source: ContextItemSource.Uri, provider: 'url' },
                            props,
                            signal
                        )) ?? Promise.resolve([])
                    )
                } catch {
                    logError('MixinContextProvider', 'Could not fetch additional URL')
                    return Promise.resolve([] as ContextItemWithContent[])
                }
            }) ?? []
        const additionalUrls = (await Promise.all(additionalUrlPromises)).flat()

        return [{ ...item, content: found.template }, ...additionalUrls]
        // .map(m => ({ ...item, content: m.template }))
        // return found
    },
}
...
const _mixins = [
    {
        ...
        emoji: '🧜‍♀️',
        additionalUrls: [
            //TODO(rnauta): We either need to post-process existing docs to remove extra tokens.
            //TODO(rnauta): Might also be an idea to add a "gist" provider to GitHub context provider
            {
                title: 'Mermaid cheatsheet',
                uri: URI.parse(
                    'https://gist.githubusercontent.com/RXminuS/90e1b63f0ff1072949fc92eab5fa0634/raw/d68de6f602870fc73ee0e30c17198d18ae7092de/mermaid-combined-cheatsheet.md'
                ),
            },
        ],
        keywords: ['visuals', 'visualize', 'visualise', 'visualisation', 'mermaid'],
        // TODO(rnauta): We need to provide guidance on when to use which graph type
        template: dedent`
            Whenever useful include a mermaid graph to help visualize the problem or your answer.

            Choose the appropriate graph type based on the question.

            Wrap the code for the graph in a "\`\`\`mermaid \`\`\`" block so it is rendered correctly in markdown.

            Mermaid is sensitive to whitespace and indentation. You must not include extra whitespace or trailing spaces if not required by the syntax. You must make sure the indentation is correct.

            After the image also include a image label by writing \`__Image **#**: short title\`. Here # is the number of the image in the conversation. Use this number to refer to the image in any text.

            In your answers always start by prov3iding the images first before adding any additional text or code.`,
    },
    {
        id: 'explore',
    ...
    },
    {
        ...
        template: dedent`For any solution or answer that you provide you must include a brief pro's and con's list.

        Each option or item to be compared must be given their own pro/con list.

        Don't use normal list bullets like * instead prefix each item with either a 🟢 (pro), 🟡 (not necessarily a con but important consideration), or 🔴 (con). Start with the pro's and end with the con's.`,
    },
    {
        id: 'eli5',
    ...
    },
    {
        ...
        template: `Keep your answers very short and direct. Don't give any examples unless asked explicitly.

        Use as few words as possible to provide an answer to any questions. Do not repeat yourself. If information is already provided in an image or graph do not repeat the information.

        If an answer requires more text then start by just providing a rough short summary and then ask the user "Do you want me to elaborate on that?".`,
    },
    {
        id: 'tdd',
