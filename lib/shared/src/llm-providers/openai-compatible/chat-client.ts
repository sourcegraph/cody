...
export async function llmChatClient({
    ...
    try {
        ...
        const { fullStream } = streamText({
            ...
            temperature: 1,
            providerOptions: {
                llm: { tool_choice: null, toolChoice: null, tools: params.tools || [] },
            },
            tools,
        ...
        while (true) {
            ...
            if (done) {
                ...
                logDebug('llmChatClient', 'done', { verbose: result })
                break
            }

            if (value.type === 'finish' || value.type === 'step-finish') {
                logDebug('llmChatClient', 'Finish received', { verbose: { result, value } })
                break
            }

            if (value.type === 'error') {
                console.error('Error from provider', value.error)
                throw value.error
            }

            if (value.type === 'text-delta') {
                result.completion += value.textDelta
                cb.onChange(result.completion, result.content)
            }
            ...
            if (value.type === 'tool-call-delta') {
                if (value.toolCallId) {
                    ...
                    const existing = activeToolCalls.get(value.toolCallId) || {
                        name: value.toolName || '',
                        args: '',
                    }
                    // Accumulate the args delta
                    existing.args += value.argsTextDelta || ''
                    activeToolCalls.set(value.toolCallId, existing)

                    // Add to our tracked tool calls for this response
                    const toolCall = {
                        id: value.toolCallId,
                        name: value.toolName || existing.name,
                        args: existing.args,
                    }
                    result.toolCalls = result.toolCalls.filter(t => t.id !== value.toolCallId)
                    result.toolCalls.push(toolCall)

                    logDebug('llmChatClient', `Updated tool call: ${toolCall.name}`, {
                        verbose: { toolCall, activeToolCalls: Array.from(activeToolCalls.entries()) },
                    })
