...
The regex pattern must precisely match the code you enclosed in each code block. The code block content will replace the matched code. For functions, always use the pattern '(functionName[\s\S]*$)' to capture from the function declaration to end of function, or '.*' to capture the entire file. Exclude the regex field for non-replaceable code blocks.
