...
export async function getFilePathContext(filePath: string): Promise<ContextMessage[]> {
    ...
    try {
        ...
        const contextFile: ContextFile = { fileName }
        contextFile.source = 'editor'

        // Make sure the truncatedContent is in JSON format
        return getContextMessageWithResponse(populateCodeContextTemplate(truncatedContent, fileName), contextFile)
    } catch (error) {
        console.error(error)
        return []
    }
...
export function getEditorSelectionContext(selection: ActiveTextEditorSelection): ContextMessage[] {
    ...
    const contextFile: ContextFile = selection
    contextFile.source = 'editor'
...
export async function getCurrentFileImportsContext(): Promise<ContextMessage[]> {
    ...
    try {
        ...
        return getContextMessageWithResponse(populateImportListContextTemplate(truncatedContent, fileName), contextFile)
    } catch {
        return []
    }
}
...
export function createHumanDisplayTextWithDocLink(
    ...
    return `${input} [_@${fileName}:${range}_](${fileLink})`
}
