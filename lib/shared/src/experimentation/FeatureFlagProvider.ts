...
export class FeatureFlagProviderImpl implements FeatureFlagProvider {
	...
	private evaluatedFeatureFlags: Observable<Record<string, boolean>> =
		combineLatest(this.relevantAuthStatusChanges, this.refreshes).pipe(
			debounceTime(0),
	 ...
	 */
	public async evaluateFeatureFlagEphemerally(
		flagName: FeatureFlag,
	): Promise<boolean> {
		return wrapInActiveSpan(
			`FeatureFlagProvider.evaluateFeatureFlag.${flagName}`,
			() => firstValueFrom(this.evaluatedFeatureFlag(flagName)),
		);
	}

	private evaluatedFeatureFlagCache: Partial<
		Record<FeatureFlag, StoredLastValue<boolean>>
	> = {};
						...
						switchMap(([authStatus]) =>
							concat(
								...
								this.evaluatedFeatureFlags.pipe(
									map((featureFlags) =>
										Boolean(featureFlags[flagName.toString()]),
									),
								),
							),
						),
