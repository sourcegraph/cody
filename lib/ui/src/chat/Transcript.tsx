...
> = React.memo(function TranscriptContent({
    ...
    const humanMessageCount = transcript.filter(
    ...
    const lastHumanMessageIndex = findLastIndex(
        transcript,
        (message) =>
            message.speaker === "human" && message.displayText !== undefined
    );
            ...
            return (
                <div>
                    ...
                    <TranscriptItem
                        ...
                        showFeedbackButtons={
                            index !== 0 && !isTranscriptError && !message.error
                        }
    ...
    return (
        ...
        >
            <div
                ref={scrollAnchoredContainerRef}
                className={classNames(styles.scrollAnchoredContainer)}
            >
                {!!chatModels?.length &&
                    ChatModelDropdownMenu &&
                    onCurrentChatModelChange &&
                ...
                {lastInteractionMessages.map(
                    messageToTranscriptItem(earlierMessages.length)
                )}
                {messageInProgress &&
                    messageInProgress.speaker === "assistant" && (
                        <TranscriptItem
                            ...
                            inProgress={
                                !!transcript[earlierMessages.length]
                                    .contextFiles
                            }
                            beingEdited={messageBeingEdited}
                            setBeingEdited={setMessageBeingEdited}
