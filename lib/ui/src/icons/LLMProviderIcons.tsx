import type React from 'react'

export const AnthropicLogo: React.FC<{ className?: string }> = ({ className }) => (
    <svg
        width="16"
        height="16"
        viewBox="0 0 16 16"
        fill="currentColor"
        className={className}
        role="img"
        aria-label="Anthropic logo"
    >
        <path d="M9.02954 2.85449L13.3201 13.6162H15.673L11.3824 2.85449H9.02954Z" />
        <path d="M4.0519 9.35765L5.52 5.57567L6.9881 9.35765H4.0519ZM4.28992 2.85449L0 13.6162H2.39866L3.27602 11.3563H7.76413L8.64134 13.6162H11.04L6.75009 2.85449H4.28992Z" />
    </svg>
)

export const OpenAILogo: React.FC<{ className?: string }> = ({ className }) => (
    <svg
        width="16"
        height="16"
        viewBox="0 0 16 16"
        fill="currentColor"
        className={className}
        role="img"
        aria-label="OpenAI logo"
    >
        <path d="M15.051 6.549a3.94 3.94 0 0 0-.347-3.274 4.106 4.106 0 0 0-4.4-1.934A4.065 4.065 0 0 0 7.257 0C5.48-.005 3.906 1.123 3.36 2.79A4.038 4.038 0 0 0 .658 4.725a3.99 3.99 0 0 0 .503 4.727 3.94 3.94 0 0 0 .347 3.274 4.106 4.106 0 0 0 4.4 1.934A4.062 4.062 0 0 0 8.955 16c1.777.004 3.353-1.125 3.9-2.793a4.038 4.038 0 0 0 2.7-1.934 3.991 3.991 0 0 0-.503-4.725Zm-6.095 8.405c-.71.001-1.4-.244-1.945-.694.025-.013.068-.036.096-.053l3.229-1.84a.517.517 0 0 0 .265-.454V7.422l1.365.777a.047.047 0 0 1 .026.037v3.72c-.002 1.654-1.36 2.995-3.036 2.998Zm-6.528-2.751a2.952 2.952 0 0 1-.363-2.01l.096.057 3.23 1.84a.532.532 0 0 0 .53 0l3.941-2.246V11.4a.05.05 0 0 1-.019.042L6.58 13.3c-1.453.825-3.31.334-4.15-1.098h-.001Zm-.85-6.955a3.019 3.019 0 0 1 1.581-1.314l-.001.11v3.68c-.001.187.1.36.265.453l3.942 2.246L6 11.2a.049.049 0 0 1-.046.004L2.69 9.344a2.98 2.98 0 0 1-1.113-4.095h.001ZM12.79 7.823 8.848 5.577l1.365-.777a.049.049 0 0 1 .046-.004l3.264 1.86a2.978 2.978 0 0 1 1.112 4.096 3.027 3.027 0 0 1-1.581 1.314v-3.79a.516.516 0 0 0-.264-.453Zm1.358-2.017a4.353 4.353 0 0 0-.096-.057l-3.229-1.84a.532.532 0 0 0-.53 0L6.35 6.155V4.6a.05.05 0 0 1 .02-.041L9.633 2.7c1.454-.828 3.312-.335 4.15 1.1.355.606.483 1.315.363 2.005ZM5.609 8.577 4.244 7.8a.047.047 0 0 1-.026-.037v-3.72c0-1.656 1.362-2.998 3.04-2.997.71 0 1.397.246 1.943.694a2.27 2.27 0 0 0-.096.054l-3.228 1.84a.516.516 0 0 0-.266.453l-.002 4.49ZM6.35 7l1.756-1 1.756 1v2l-1.756 1L6.35 9V7Z" />
    </svg>
)

export const MistralLogo: React.FC<{ className?: string }> = ({ className }) => (
    <svg
        width="16"
        height="16"
        viewBox="0 0 16 16"
        fill="currentColor"
        className={className}
        role="img"
        aria-label="Mistral logo"
    >
        <rect x="13" y="0.5" width="3" height="3" />
        <rect x="1" y="0.5" width="3" height="3" />
        <rect opacity="0.85" x="13" y="3.5" width="3" height="3" />
        <rect x="13" y="6.5" width="3" height="3" fillOpacity="0.7" />
        <rect x="10" y="6.5" width="3" height="3" fillOpacity="0.7" />
        <rect x="7" y="6.5" width="3" height="3" fillOpacity="0.7" />
        <rect x="4" y="6.5" width="3" height="3" fillOpacity="0.7" />
        <rect x="1" y="6.5" width="3" height="3" fillOpacity="0.7" />
        <rect x="1" y="9.5" width="3" height="3" fillOpacity="0.55" />
        <rect x="7" y="9.5" width="3" height="3" fillOpacity="0.55" />
        <rect x="13" y="9.5" width="3" height="3" fillOpacity="0.55" />
        <rect x="13" y="12.5" width="3" height="3" fillOpacity="0.4" />
        <rect x="1" y="12.5" width="3" height="3" fillOpacity="0.4" />
        <rect opacity="0.85" x="10" y="3.5" width="3" height="3" />
        <rect opacity="0.85" x="1" y="3.5" width="3" height="3" />
        <rect opacity="0.85" x="4" y="3.5" width="3" height="3" />
        <rect x="12" y="9.5" width="1" height="6" fill="#1B1B1B" />
        <rect x="12" y="0.5" width="1" height="3" fill="#1B1B1B" />
        <rect x="9" y="3.5" width="1" height="3" fill="#1B1B1B" />
        <rect x="6" y="9.5" width="1" height="3" fill="#1B1B1B" />
        <rect y="0.5" width="1" height="15" fill="#1B1B1B" />
    </svg>
)

export const MetaLogo: React.FC<{ className?: string }> = ({ className }) => (
    <svg
        width="16"
        height="16"
        viewBox="0 0 32 32"
        fill="currentColor"
        className={className}
        role="img"
        aria-label="Meta logo"
    >
        <path d="M5,19.5c0-4.6,2.3-9.4,5-9.4c1.5,0,2.7,0.9,4.6,3.6c-1.8,2.8-2.9,4.5-2.9,4.5c-2.4,3.8-3.2,4.6-4.5,4.6  C5.9,22.9,5,21.7,5,19.5 M20.7,17.8L19,15c-0.4-0.7-0.9-1.4-1.3-2c1.5-2.3,2.7-3.5,4.2-3.5c3,0,5.4,4.5,5.4,10.1  c0,2.1-0.7,3.3-2.1,3.3S23.3,22,20.7,17.8 M16.4,11c-2.2-2.9-4.1-4-6.3-4C5.5,7,2,13.1,2,19.5c0,4,1.9,6.5,5.1,6.5  c2.3,0,3.9-1.1,6.9-6.3c0,0,1.2-2.2,2.1-3.7c0.3,0.5,0.6,1,0.9,1.6l1.4,2.4c2.7,4.6,4.2,6.1,6.9,6.1c3.1,0,4.8-2.6,4.8-6.7  C30,12.6,26.4,7,22.1,7C19.8,7,18,8.8,16.4,11" />
    </svg>
)

export const UnrecognizedLogo: React.FC<{ name: string; abbreviation: string; className?: string }> = ({
    name,
    abbreviation,
    className,
}) => (
    // svg that displays the text in `label`
    <svg
        width="16"
        height="16"
        viewBox="0 0 16 16"
        fill="currentColor"
        className={className}
        role="img"
        aria-label={`${name} logo`}
    >
        <g>
            <rect width="16" height="16" fill="transparent" stroke="#ffffff77" />
            <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle" fontSize="90%">
                {abbreviation}
            </text>
        </g>
    </svg>
)
