    ...
    return {
        type(mention: string) {
            if (previousState !== getEditorState(editor)) {
                throw new Error('Editor state has changed since insertion of at-mention. Cannot continue.')
            }
...
describe('generic document editing', () => {
    test('document.set', () => {
        ...
        editor.send({type: 'document.set', doc: schema.node('doc', null, schema.node('paragraph', null, [schema.text('new')]))})
...
describe('mentions', () => {
    describe('document.mentions.add', () => {
        test('append', () => {
            ...
            input.send({type: 'document.mentions.add', items: [cm('file2'),cm('file3')], position: 'after', separator: ' ! '})
        ...
        test('prepend', () => {
            const input = createInput(['before ', cm('file1'), ' after'])
            input.send({type: 'document.mentions.add', items: [cm('file2'),cm('file3')], position: 'before', separator: ' ! '})
        ...
        test('update mention', () => {
            ...
            input.send({type: 'document.mentions.add', items: [cm('file1', [0,6]), cm('file2', [4,10])], position: 'after', separator: ' ! '})
...
describe('mentions menu', () => {
    ...
    test('debounces fetching of menu data', () => {
        ...
        expectedQueries.forEach((query, index) => {
            expect.soft(fetchMenuData).toHaveBeenNthCalledWith(index + 1, expect.objectContaining({input: expect.objectContaining({query})}))
        })
    })
    ...
    test('menu items are updated according to currently available context size', () => {
        ...
        editor.send({type: 'document.mentions.add', items: [{type: 'file', uri: 'file:///file3.txt', size: 5, source: ContextItemSource.User}], position: 'before', separator: ' '})
