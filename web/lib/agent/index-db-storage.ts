...
export class IndexDBStorage implements vscode.Memento {
    ...
    async get(key: string, defaultValue?: any): Promise<any> {
        const store = this.db.transaction(IndexDBStorage.DATABASE_STORE_NAME).objectStore(IndexDBStorage.DATABASE_STORE_NAME)
    ...
    async update(key: string, value: any): Promise<void> {
        const store = this.db.transaction(IndexDBStorage.DATABASE_STORE_NAME, "readwrite").objectStore(IndexDBStorage.DATABASE_STORE_NAME)
