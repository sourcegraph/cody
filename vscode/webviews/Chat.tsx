...
> = ({
	...
	const [isEnhancedContextOpen, setIsEnhancedContextOpen] =
		useState(isNewInstall);
	const [isResponseSettingsOpen, setIsResponseSettingsOpen] = useState(false);

	const editorRef = useRef<PromptEditorRefAPI>(null);
	const setEditorState = useCallback(
		(state: SerializedPromptEditorState | null) => {
	...
	const postMessage = useCallback<ApiPostMessage>(
		(msg) => vscodeAPI.postMessage(msg),
		[vscodeAPI],
	);
	...
	const setEditMessageState = useCallback(
		(index?: number): void => {
			...
			if (messageAtIndex) {
				setEditorState(
					serializedPromptEditorStateFromChatMessage(messageAtIndex),
				);
			}
					...
					<div className={styles.editorOuterContainer}>
						...
						<div className={styles.editorSuffixButtons}>
							<EnhancedContextSettings
								...
								presentationMode={
									userInfo.isDotComUser ? "consumer" : "enterprise"
								}
...
const SubmitButton: React.FunctionComponent<ChatUISubmitButtonProps> = ({
	...
	<VSCodeButton
		...
		title={
			onAbortMessageInProgress
				? submitButtonTypes.abort.title
				: submitButtonTypes[type]?.title
		}
	>
		<i
			className={
				onAbortMessageInProgress
					? submitButtonTypes.abort.icon
					: submitButtonTypes[type]?.icon
			}
		/>
	</VSCodeButton>
);
