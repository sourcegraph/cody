...
export const AssistantMessageCell: FunctionComponent<{
  ...
  ({
    ...
    const hasLongerResponseTime = chatModel?.tags?.includes(
                ...
                <div>
                  {hasLongerResponseTime && (
                    <p className="tw-m-4 tw-mt-0 tw-text-muted-foreground">
                      This model may take longer to respond because it takes
                      time to "think". Recommended for complex reasoning &
                      coding tasks.
                    </p>
                  )}
                  <LoadingDots />
...
export const NON_HUMAN_CELL_AVATAR_SIZE =
  MESSAGE_CELL_AVATAR_SIZE *
  0.83; /* make them "look" the same size as the human avatar icons */
...
export interface PriorHumanMessageInfo {
  ...
  rerunWithDifferentContext: (
    withInitialContext: HumanMessageInitialContextInfo,
  ) => void;
  ...
  return {
    ...
    hasInitialContext: {
      ...
      files: Boolean(
        contextItems.some(
          (item) =>
            item.type === "file" && item.source === ContextItemSource.Initial,
        ),
      ),
    },
    ...
    hasExplicitMentions: Boolean(
      contextItems.some((item) => item.source === ContextItemSource.User),
    ),
    appendAtMention: () => {
      if (
        humanEditorRef.current?.getSerializedValue().text.trim().endsWith("@")
      ) {
...
function useChatModelByID(
  ...
  return (
    ...
    (model
      ? {
          id: model,
          title: model?.includes(DeepCodyAgentID)
            ? "Deep Cody (Experimental)"
            : model,
          provider: "unknown",
          tags: [],
        }
