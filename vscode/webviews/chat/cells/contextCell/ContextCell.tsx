...
export const ContextCell: React.FunctionComponent<{
	...
	const fileCount = new Set(usedContext.map((file) => file.uri.toString()))
		.size;
	let fileCountLabel = `${fileCount} file${fileCount > 1 ? "s" : ""}`;
	if (excludedAtContext.length) {
		const excludedAtUnit =
			excludedAtContext.length === 1 ? "mention" : "mentions";
				...
				<details className={styles.details} open={__storybook__initialOpen}>
					...
					>
						<h4 className={styles.heading}>
							Context{" "}
							<span className={styles.stats}>&mdash; {fileCountLabel}</span>
						</h4>
					</summary>
