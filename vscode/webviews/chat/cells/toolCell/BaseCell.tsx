...
export interface BaseCellProps {
    ...
    /** Background color for the body content */
    bodyBgClass?: string
}
...
export const BaseCell: FC<BaseCellProps> = ({
    ...
    isLoading = false,
    defaultOpen = false,
    headerBgClass = 'tw-bg-zinc-900',
    bodyBgClass = 'tw-bg-zinc-950',
}) => {
    ...
    return (
        <div className={cn('tw-rounded-md tw-border tw-border-border tw-w-full', className)}>
            <Collapsible open={isOpen} onOpenChange={setIsOpen} className="tw-w-full tw-dark">
                <CollapsibleTrigger
                    className={cn(
                        'tw-flex tw-w-full tw-items-center tw-justify-between tw-px-4 tw-py-2 tw-text-sm tw-text-zinc-100 hover:tw-bg-zinc-800',
                        headerBgClass,
