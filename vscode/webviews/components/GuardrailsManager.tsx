...
class GuardrailsCache {
    ...
    private readonly cache = new WeakMap<
        Guardrails,
        {
            ...
            attributionRequests: LRUCache<string, ReturnType<Guardrails['searchAttribution']>>
    ...
    searchAttribution(
        guardrails: Guardrails,
        code: string
    ): ReturnType<Guardrails['searchAttribution']> {
        ...
        if (!result) {
            result = guardrails.searchAttribution(code)
            cache.attributionRequests.set(code, result)
            result
                .then(attributionOrError => {
                    if (cache.attributionRequests.get(code) === result) {
                        cache.results.set(code, parseAttributionResult(attributionOrError))
                    }
                })
                .finally(() => {
                    if (cache.attributionRequests.get(code) === result) {
                        // This request is done, so clean up the cache.
...
export const GuardrailsApplicator: React.FC<GuardrailsApplicatorProps> = ({
    ...
    useEffect(() => {
        if (isCodeComplete) {
            ...
            guardrailsCache.searchAttribution(guardrails, code).then(result => {
                setGuardrailsResult(parseAttributionResult(result))
            })
