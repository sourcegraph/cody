...
function getTooltip(model: Model, availability: string): string {
    if (model.id.includes('deep-cody')) {
        return 'An AI agent powered by Claude 3.5 Sonnet (New) and other models with tool-use capabilities to gather contextual information for better responses. It can search your codebase, browse the web, execute shell commands in your terminal (when enabled), and utilize any configured tools to retrieve necessary context. To enable shell commands, set the "cody.experimental.agenticContext.shell" option to true in your settings.'
    }
