import { ChevronLeft, ChevronRight, Cloud, Key, Search, Server } from 'lucide-react'
...
import { type ApiKey, type ServerType, initialApiKeys, initialServers } from '../types'
...
export function ServerDashboard() {
    ...
    // Add new API key
    const addApiKey = (newKey: Omit<ApiKey, 'id' | 'created' | 'lastUsed'>) => {
        const apiKey: ApiKey = {
            ...newKey,
        ...
        >
                        ...
                        <div className="tw-w-full">
                            ...
                            {activeTab === 'api-keys' && (
                                <div className="tw-mt-0">
                                    <div className="tw-flex tw-justify-between tw-items-center tw-px-4 tw-py-2">
                                        <div className="tw-flex tw-items-center tw-gap-2">
                                            <span className="tw-text-sm tw-font-medium">API Keys</span>
                                            <Badge variant="outline" className="tw-text-xs">
                                                {apiKeys.length}
                                            </Badge>
                                        </div>
                                        <AddApiKeyDialog onAddApiKey={addApiKey} />
                                    </div>
                                    ...
                                    <ScrollArea className="tw-h-[calc(100vh-180px)]">
                                        <div className="tw-px-2 tw-py-1">
                                            ...
                                            {filteredApiKeys.length === 0 && (
                                                <div className="tw-text-center tw-py-8 tw-text-muted-foreground">
                                                    <Key className="tw-h-8 tw-w-8 tw-mx-auto tw-mb-2 tw-opacity-50" />
            ...
            <div className={cn('tw-flex-1 tw-overflow-auto', isSidebarView ? '' : '')}>
                {activeTab === 'servers' ? (
                    ...
                    <ApiKeysView apiKeys={filteredApiKeys} isSidebarView={isSidebarView} />
                )}
            </div>
        </div>
    )
