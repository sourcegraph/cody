...
export function deserializeContextItem(
...
function convertContextItemMentionElement(
	domNode: HTMLElement,
...
export class ContextItemMentionNode extends TextNode {
	...
	static importJSON(
		serializedNode: SerializedContextItemMentionNode,
	...
	private static CLASS_NAMES =
		`context-item-mention-node ${styles.contextItemMentionNode}`;
...
export function contextItemMentionNodeDisplayText(
	contextItem: SerializedContextItem,
): string {
	...
	const rangeText = contextItem.range
		? `:${displayLineRange(contextItem.range)}`
		: "";
	if (contextItem.type === "file") {
		return `@${decodeURIComponent(
			displayPath(URI.parse(contextItem.uri)),
		)}${rangeText}`;
	}
...
export function $createContextItemTextNode(
	contextItem: ContextItem | SerializedContextItem,
): TextNode {
