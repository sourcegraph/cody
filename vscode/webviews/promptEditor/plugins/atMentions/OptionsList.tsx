...
const FileItem: FunctionComponent<ItemProps<ContextItemFile>> = ({
	...
	const description = `${range ? `Lines ${range} · ` : ""}${
		dir === "." ? "" : dir
	}`;
	// =======
	...
	const warning =
		isLargeFile && !item.range && !isValidLineRangeQuery(query)
			? LARGE_FILE_WARNING_LABEL
			: "";
...
const SymbolItem: FunctionComponent<ItemProps<ContextItemSymbol>> = ({
	...
	const description = `${displayPath(item.uri)}:${getLineRangeInMention(
		query,
		item.range,
	)}`;
	...
	return (
		...
		>
			<div className={styles.optionItemRow}>
				...
				<span className={classNames(styles.optionItemTitle)}>
					{provider.triggerPrefixes[0]}
				</span>
...
function getLineRangeInMention(query: string, range?: RangeData): string {
	...
	if (query && queryRange?.[1]) {
		...
		return `${startLine}-${
			endLine !== Number.POSITIVE_INFINITY ? endLine : "#"
		}`;
	}
