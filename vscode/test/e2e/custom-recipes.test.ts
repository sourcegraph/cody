...
test('open the Custom Commands in sidebar and add new user recipe', async ({ page, sidebar }) => {
    ...
    // Check if the Custom Commands UI shows up
    await expect(sidebar.getByText('Custom Commands')).toBeVisible()
    ...
    await expect(sidebar.getByTitle('Open Custom Commands Menu')).toBeVisible()
    await sidebar.getByTitle('Open Custom Commands Menu').click()
    await page.getByText('Cody: Custom Commands (Experimental)').click()
