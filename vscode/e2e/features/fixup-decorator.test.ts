...
test.describe('fixup decorator', testDetails, () => {
    test('decorations from un-applied Cody changes appear', async ({
        ...
        await vsc.editor.openFile({ workspaceFile: 'index.html' })
        ...
        await test.step('trigger decorations with edit', async () => {
            ...
            await vsc.QuickPick.input.fill('Replace hello with goodbeye', { force: true })
            const submit = vsc.QuickPick.items({ hasText: /Submit/ })
        ...
        await test.step('modify and navigate text to invalidate decorations', async () => {
            await vsc.editor.active.pressSequentially('Hello World')
            await vsc.editor.active.press('ArrowRight')
        })
