...
test.describe('cody.at-mention', () => {
    ...
    for (const repoVariant of repoVariants) {
        for (const endpointVariant of endpointVariants) {
            test(
                `mention events fire correctly (${repoVariant}, ${endpointVariant})`,
                { tag: [`@${endpointVariant}`, `@${repoVariant}-repo`] },
                async (
                    { page, mitmProxy, vscodeUI, workspaceDir, telemetryRecorder, polly, context },
                    testInfo
                ) => {
                    ...
                    await uix.mitm.withFloorResponseTime(120 * 1000, { mitmProxy }, async () => {
                        ...
                        await expect(
                            chat.content.locator('[data-testid="message"]').nth(2)
                        ).toBeVisible()
