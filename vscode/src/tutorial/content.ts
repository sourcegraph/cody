...
type TutorialStep = 'autocomplete' | 'edit' | 'fix' | 'chat'

const getStep = (step: TutorialStep): string => {
    switch (step) {
        case 'autocomplete':
            return dedent`
                ### Welcome to Cody!
...
export const getOsSpecificStep = (step: TutorialStep): string => {
    const stepContent = getStep(step)
    return stepContent.replace('Opt', 'Alt').replace('Cmd', 'Ctrl')
}
