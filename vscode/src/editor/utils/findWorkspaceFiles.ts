...
async function getExcludePattern(workspaceFolder: vscode.WorkspaceFolder): Promise<string> {
    ...
    const gitignoreExclude = useIgnoreFiles
        ? await readIgnoreFile(vscode.Uri.joinPath(workspaceFolder.uri, '.gitignore'))
        : {}
    const ignoreExclude = useIgnoreFiles
