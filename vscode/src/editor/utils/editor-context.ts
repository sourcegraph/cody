...
export async function getFileContextFiles(
	...
	const adjustedResults = [...results].map((result) => {
		const segments = result.obj.uri.path
			.split(/[\/\\]/)
			.filter((segment) => segment !== "");
		for (const lowScoringPathSegment of lowScoringPathSegments) {
			if (
				segments.includes(lowScoringPathSegment) &&
				!query.includes(lowScoringPathSegment)
			) {
				return {
					...result,
	...
	const sortedResults = adjustedResults
		...
		.flatMap((result) =>
			createContextFileFromUri(result.obj.uri, ContextItemSource.User, "file"),
		);
...
export async function filterContextItemFiles(
	...
	for (const cf of contextFiles) {
		...
		cf.size = Math.floor(
			fileStat.size / (cf.uri.fsPath.endsWith(".md") ? 3.5 : 4.5),
		);
		filtered.push(cf);
	}
	return filtered;
...
export async function resolveContextItems(
	...
	return (
		await Promise.all(
			items.map(
				async (item: ContextItem): Promise<ContextItemWithContent[] | null> => {
					try {
...
async function resolveContextMentionProviderContextItem(
	...
	for (const provider of getEnabledContextMentionProviders()) {
		if (provider.id === itemProvider && provider.resolveContextItem) {
			return provider.resolveContextItem(
...
async function resolveFileOrSymbolContextItem(
	...
	const content =
		contextItem.content ??
		(await editor.getTextEditorContentForFile(
			contextItem.uri,
