...
export class VSCodeEditorContext {
    ...
    public getHumanDisplayText(humanInput: string): string {
        if (!this.selection?.fileUri) {
            return humanInput
        }

        if (!this.editor.getWorkspaceRootUri()) {
            return humanInput + displayFileName + this.selection.fileUri.toString()
        }

        // Create markdown link to the file
        return createHumanDisplayTextWithDocLink(humanInput, this.selection.fileUri, this.selection)
    }
    ...
    public getCurrentFileContextFromEditorSelection(): ContextMessage[] {
        ...
        return getContextMessageWithResponse(
            populateCurrentFileFromEditorSelectionContextTemplate(this.selection, this.selection.fileUri),
            {
                type: 'file',
                uri: this.selection.fileUri,
                repoName: this.selection.repoName,
    ...
    public async getPackageJsonContext(): Promise<ContextMessage[]> {
