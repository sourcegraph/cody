...
export class AuthProvider {
  ...
  public async init(): Promise<void> {
    let lastEndpoint =
      localStorage?.getEndpoint() || this.config.serverEndpoint;
    let token =
      (await secretStorage.get(lastEndpoint || '')) || this.config.accessToken;
    logDebug(
      'AuthProvider:init',
      token?.trim()
        ? 'Token recovered from secretStorage'
        : 'No token found in secretStorage'
    );
  ...
  public async accountMenu(): Promise<void> {
    ...
    const detail = `Plan: ${
      this.authStatus.userCanUpgrade ? 'Cody Free' : 'Cody Pro'
    }`;
  ...
  private async makeAuthStatus(
    ...
    const [{ enabled, version }, codyLLMConfiguration, userInfo] =
      await Promise.all([
    ...
    const configOverwrites = isError(codyLLMConfiguration)
      ? undefined
  ...
  public redirectToEndpointLogin(uri: string): void {
    ...
    if (vscode.env.uiKind === vscode.UIKind.Web) {
      ...
      const newTokenNoCallbackUrl = new URL(
        '/user/settings/tokens/new',
        endpoint
      );
      void vscode.env.openExternal(
        vscode.Uri.parse(newTokenNoCallbackUrl.href)
      );
      void this.signinMenuForInstanceUrl(endpoint);
      return;
    }

    const newTokenCallbackUrl = new URL(
...
async function showAuthResultMessage(
  ...
  if (authStatus?.isLoggedIn) {
    ...
    await vscode.window.showInformationMessage(
      `Signed in to ${authority || endpoint}`
    );
  } else {
    await showAuthFailureMessage(endpoint);
  }
