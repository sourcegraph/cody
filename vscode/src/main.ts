...
} from "./chat/chat-view/ChatManager";
...
} from "./commands/CommandsController";
import {
    executeDocCommand,
...
import {
    createOrUpdateEventLogger,
    telemetryService,
} from "./services/telemetry";
import {
    createOrUpdateTelemetryRecorderProvider,
    telemetryRecorder,
} from "./services/telemetry-v2";
        ...
        vscode.workspace.onDidChangeConfiguration(async (event) => {
            if (event.affectsConfiguration("cody")) {
                ...
                if (config.chatPreInstruction) {
                    PromptMixin.addCustom(
                        newPromptMixin(config.chatPreInstruction)
                    );
                }
            }
        })
...
const register = async (
    ...
    disposables.push(
        ...
        vscode.window.registerWebviewViewProvider(
            "cody.chat",
            chatManager.sidebarViewController,
            {
    ...
    disposables.push(
                ...
                if (upgradeAvailable) {
                    ...
                    if (option) {
                        void vscode.env.openExternal(
                            vscode.Uri.parse(ACCOUNT_UPGRADE_URL.toString())
                        );
                    }
                } else {
                    const option = await vscode.window.showInformationMessage(
        ...
        vscode.window.registerUriHandler({
            handleUri: async (uri: vscode.Uri) => {
                if (uri.path === "/app-done") {
                    ...
                    await authProvider.tokenCallbackHandler(
                        uri,
        ...
        vscode.commands.registerCommand(
            ...
            (name: string, command: string) => {
                ...
                telemetryService.log(
                    `CodyVSCodeExtension:command:${name}:clicked`,
                    { source }
                );
                telemetryRecorder.recordEvent(
                    `cody.command.${name}`,
        ...
        // For debugging
        vscode.commands.registerCommand("cody.debug.export.logs", () =>
            exportOutputLog(context.logUri)
        ),
        vscode.commands.registerCommand("cody.debug.outputChannel", () =>
            openCodyOutputChannel()
        ),
        vscode.commands.registerCommand("cody.debug.enable.all", () =>
            enableDebugMode()
        )
    );
        ...
        setupAutocompleteQueue = setupAutocompleteQueue
            .then(async () => {
                ...
                const autocompleteFeatureFlagChangeSubscriber =
                    featureFlagProvider.onFeatureFlagChanged(
                        "cody-autocomplete",
                        setupAutocomplete
                    );
            ...
            .catch((error) => {
                console.error(
                    "Error creating inline completion item provider:",
                    error
                );
            });
...
async function configureEventsInfra(
    ...
    await createOrUpdateTelemetryRecorderProvider(
        config,
