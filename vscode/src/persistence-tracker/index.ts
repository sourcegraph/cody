...
import { PersistencePresentEventPayload, PersistenceRemovedEventPayload, TrackedInsertion } from './types'
...
export class PersistenceTracker<T = string> implements vscode.Disposable {
    ...
    public track({
        ...
        // The range for the insertion is relative to the state before the insertion was applied.
        // We need to convert it to the state after the insertion was applied.
    ...
    private measure(
        ...
        const isStillTracked = this.trackedInsertions.get(trackedInsertion.uri.toString())?.has(trackedInsertion)
        if (!isStillTracked) {
            return
        }
        ...
        if (latestText.length === 0) {
            ...
            this.logger.onPresent({
                ...
                difference,
                lineCount: trackedInsertion.latestRange.end.line - trackedInsertion.latestRange.start.line + 1,
    ...
    private onDidChangeTextDocument(event: vscode.TextDocumentChangeEvent): void {
        ...
        for (const trackedInsertion of documentInsertions) {
            trackedInsertion.latestRange = updateRangeMultipleChanges(trackedInsertion.latestRange, mutableChanges)
        }
    }
