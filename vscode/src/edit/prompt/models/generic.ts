...
const GENERIC_PROMPTS: Record<EditIntent, PromptVariant> = {
    ...
    test: {
        instruction: psDedent`
            ...
            - Your response must start with the suggested file path relative to the workspace between <${PROMPT_TOPICS.WORKSPACE_RELATIVE_FILEPATH}> XML tags, ensuring it aligns with the directory structure and conventions from the shared context`,
    },
    doc: {
        system: psDedent`
            - You are an AI programming assistant who is an expert in updating code to meet given instructions.
