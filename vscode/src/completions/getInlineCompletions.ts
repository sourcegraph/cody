...
async function doGetInlineCompletions({
    ...
    documentHistory,
    requestManager,
    lastInlineCompletionResult: lastInlineCompletionItemResult,
    setIsLoading,
    abortSignal,
    tracer,
    ...
    if (lastInlineCompletionItemResult) {
        const isSameDocument = lastInlineCompletionItemResult.uri.toString() === document.uri.toString()
        const isSameLine = lastInlineCompletionItemResult.originalTriggerPosition.line === position.line
        const isPastOriginalTriggerPosition = position.isAfterOrEqual(
            lastInlineCompletionItemResult.originalTriggerPosition
        )
        const isSamePrefix = lastInlineCompletionItemResult.firstLineFullText.startsWith(docContext.currentLinePrefix)
        if (isSameDocument && isSameLine && isPastOriginalTriggerPosition && isSamePrefix) {
            return {
                // Reuse the logId to so that typing text of a displayed completion will not log a
                // new completion on every keystroke.
                logId: lastInlineCompletionItemResult.logId,

                items: [
                    {
                        insertText: lastInlineCompletionItemResult.firstLineFullText.slice(
    ...
    const { logId: completions, cacheHit } = await requestManager.request(
