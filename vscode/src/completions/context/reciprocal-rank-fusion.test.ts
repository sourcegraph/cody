...
describe('fuseResults', () => {
    it('fuses multiple context item lists into one ranked list', () => {
        ...
        const rankingIdentity = (item: { id: number }) => [item.id.toString()]

        const results = fuseResults(retrievers, rankingIdentity)
    ...
    it('handles the same result being part of multiple documents without getting duplicated', () => {
        ...
        const rankingIdentity = (item: { lines: number[] }) => item.lines.map(id => id.toString())
