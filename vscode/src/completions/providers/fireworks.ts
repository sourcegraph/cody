...
class FireworksProvider extends Provider {
	...
	private authStatus: Pick<
		AuthStatus,
		"userCanUpgrade" | "isDotCom" | "endpoint"
	>;
	...
	private createPrompt(snippets: ContextSnippet[]): string {
		...
		for (
			let snippetsToInclude = 0;
			snippetsToInclude < snippets.length + 1;
			snippetsToInclude++
		) {
			if (snippetsToInclude > 0) {
				...
				if ("symbol" in snippet && snippet.symbol !== "") {
					...
					intro.push(
						`Here is a reference snippet of code from ${displayPath(
							snippet.uri,
						)}:\n\n${snippet.content}`,
					);
				}
			...
			const introString = `${intro
				...
				.split("\n")
				.map((line) =>
					languageConfig ? languageConfig.commentStart + line : "// ",
				)
				.join("\n")}\n`;
 ...
 ${OPENING_CODE_TAG}${infillBlock}`;
	...
	private async *createFastPathClient(
		...
		if (response.status === 429) {
			...
			throw await createRateLimitErrorFromResponse(
				response,
				upgradeIsAvailable,
			);
		}
		...
		try {
			...
			for await (const { event, data } of iterator) {
				...
				lastResponse = {
					completion:
						(lastResponse ? lastResponse.completion : "") + choice.text,
					stopReason:
