...
describe('getArtificialDelay', () => {
    ...
    it('returns gradually increasing latency up to max for CSS when suggestions are rejected', () => {
        ...
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1000)
        // start at default, but gradually increasing latency after 5 rejected suggestions
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1200)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1400)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1600)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1800)
        // max latency at 2000
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(2000)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(2000)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(2000)
    ...
    it('returns increasing latency after rejecting suggestions', () => {
        ...
        // start at default, but gradually increasing latency after 5 rejected suggestions
        expect(getArtificialDelay(featureFlags, uri, languageId, 'class')).toBe(0)
        ...
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(0)
        // gradually increasing latency after 5 rejected suggestions
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(200)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(400)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(600)
        // after the suggestion was accepted, user latency resets to 0, using baseline only
        resetArtificialDelay()
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(0)
    })
    ...
    it('returns increasing latency up to max after rejecting multiple suggestions, resets after file change and accept', () => {
        ...
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(600)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(800)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1200)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1400)
        expect(getArtificialDelay(featureFlags, uri, languageId, undefined)).toBe(1600)
        ...
        expect(getArtificialDelay(featureFlags, newUri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(featureFlags, newUri, languageId, undefined)).toBe(0)
        ...
        // Latency will not reset before 5 minutes
        vi.advanceTimersByTime(3 * 60 * 1000)
        expect(getArtificialDelay(featureFlags, newUri, languageId, undefined)).toBe(200)
        // reset latency on accepted suggestion
        resetArtificialDelay()
        expect(getArtificialDelay(featureFlags, newUri, languageId, undefined)).toBe(0)
    })
    ...
    it('returns increased latency for user-based only when only user flag is enabled', () => {
        ...
        // reject the first 5 suggestions, and confirm latency remains unchanged
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(0)
        // latency should start increasing after 5 rejections, but max at 2000
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(200)
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(400)
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(600)
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(800)
        expect(getArtificialDelay(featureFlagsUserOnly, uri, languageId, undefined)).toBe(1000)
    })
    ...
    it('returns default latency for low performance language only when only language flag is enabled', () => {
        ...
        expect(getArtificialDelay(featureFlagsLangOnly, goUri, languageId, undefined)).toBe(0)
    })
    ...
    it('returns increased latency for user-based only when language flag is disabled', () => {
        ...
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(0)
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(0)
        // latency for user should start increasing after 5 rejections gradually
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(200)
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(400)
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(600)
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(800)
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(1000)
        // reset to starting point on every accepted suggestion
        resetArtificialDelay()
        expect(getArtificialDelay(providerAndUserFlags, uri, languageId, undefined)).toBe(0)
    })
    ...
    it('returns latency based on language only when user flag is disabled', () => {
        ...
        expect(getArtificialDelay(featureFlagsNoUser, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlagsNoUser, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlagsNoUser, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlagsNoUser, uri, languageId, undefined)).toBe(1000)
        expect(getArtificialDelay(featureFlagsNoUser, uri, languageId, undefined)).toBe(1000)
