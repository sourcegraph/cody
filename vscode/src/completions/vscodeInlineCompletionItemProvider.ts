...
import { getInlineCompletions, InlineCompletionsParams } from './getInlineCompletions'
...
export class InlineCompletionItemProvider implements vscode.InlineCompletionItemProvider {
    ...
    private readonly config: Required<InlineCompletionItemProviderConfig>
    ...
    constructor({
        ...
        tracer = null,
        ...config
    }: InlineCompletionItemProviderConfig) {
        this.config = {
            ...config,
            responsePercentage,
            prefixPercentage,
    ...
    public async provideInlineCompletionItems(
        ...
        const logId = '_' //  TODO(sqs)
        return { items: result ? processInlineCompletionsForVSCode(logId, document, position, result.items) : [] }
    }
}
...
function processInlineCompletionsForVSCode(
    ...
    return items.map(completion => {
        ...
        return new vscode.InlineCompletionItem(
            document.getText(currentLine.range.with({ end: position })) + completion.insertText,
 ...
 * {@link InlineCompletionItemProvider.provideInlineCompletionItems} that accumulates all of the data
 * for that invocation.
 */
