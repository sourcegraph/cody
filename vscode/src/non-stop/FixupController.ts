        ...
        vscode.Disposable
{
    ...
    private async applyTask(task: FixupTask): Promise<void> {
        ...
        const editOk = await this.replaceEdit(editor, diff)
    ...
    private async revertTask(task: FixupTask): Promise<void> {
        ...
        // TODO: See if we can discard a FixupFile now.
        this.setTaskState(task, CodyTaskState.done)
    }
    ...
    private cancel(id: taskID): void {
        ...
        this.setTaskState(task, task.state === CodyTaskState.error ? CodyTaskState.error : CodyTaskState.done)
        this.discard(task)
    }
    ...
    public textDidChange(task: FixupTask): void {
        ...
        if (task.state === CodyTaskState.done || task.state === CodyTaskState.applied) {
            this.needsDiffUpdate_.delete(task)
        }
