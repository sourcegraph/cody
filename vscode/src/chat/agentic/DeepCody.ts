...
export class DeepCodyAgent {
     ...
     * @param chatAbortSignal - An AbortSignal to cancel the operation.
    ...
    public async getContext(chatAbortSignal: AbortSignal, loop = 2): Promise<ContextItem[]> {
        ...
        logDebug(
            'DeepCody',
            `added ${contextCount} agentic context in ${loopCount} loops, taking ${duration}ms`
        )
    ...
    private async review(chatAbortSignal: AbortSignal): Promise<ContextItem[]> {
        ...
        try {
            ...
            await this.multiplexer.notifyTurnComplete()
            logDebug('DeepCody', `context review failed: ${error}`, {
    ...
    private buildPrompt(): PromptString {
        return toolsPrompt
            .replace('{{CODY_TOOLS_PLACEHOLDER}}', join(this.tools.map(t => t.getInstruction())))
            .replace(
...
const toolsPrompt = ps`Your task is to review the shared context and think step-by-step to determine if you can answer the "Question:" at the end.
[INSTRUCTIONS]
