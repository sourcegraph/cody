...
export class AgenticAnthropicHandler extends ChatHandler implements AgentHandler {
    ...
    constructor(
        ...
        if (AgenticAnthropicHandler.SYSTEM_PROMPT === '') {
            AgenticAnthropicHandler.SYSTEM_PROMPT = getClaudeSystemPrompt()
        }
    }
    ...
    public async handle(req: AgentRequest, delegate: AgentHandlerDelegate): Promise<void> {
        ...
        const messages = AgenticAnthropicHandler.sessions.get(chatBuilder.sessionID) ?? []
        ...
        const requestParams = model.includes('3-5') ? defaultParams['3.5'] : defaultParams['3.7']
                    ...
                    .stream(
                        {
                            ...
                            max_tokens: 8000,
                            messages,
                            system: AgenticAnthropicHandler.SYSTEM_PROMPT,
                            stream: true,
                            ...requestParams,
                        },
                        {
        ...
        while (this.turnCount < this.MAX_TURN) {
            ...
            for (const toolCall of currentToolCalls) {
                ...
                try {
                    ...
                    logDebug('AgenticAnthropicHandler', `Failed to invoke ${toolCall.name}`, {
                        verbose: error,
                    })
                    toolResults.push({
                        type: 'tool_result',
