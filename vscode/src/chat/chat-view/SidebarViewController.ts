...
import {
  OnboardingExperimentBranch,
  logExposure,
  pickBranch,
} from "../../services/OnboardingExperiment";
import { addWebviewViewHTML } from "./ChatManager";

export interface SidebarChatWebview
  extends Omit<vscode.Webview, "postMessage"> {
...
export class SidebarViewController implements vscode.WebviewViewProvider {
  ...
  private async onDidReceiveMessage(message: WebviewMessage): Promise<void> {
    switch (message.command) {
      ...
      case "auth": {
        ...
        if (message.authKind === "simplified-onboarding") {
          ...
          if (this.startTokenReceiver) {
            logExposure();
            if (
              branch === OnboardingExperimentBranch.RemoveAuthenticationStep
            ) {
              closeAuthProgressIndicator();
              startAuthProgressIndicator();
          ...
          const successfullyOpenedUrl =
            await authProviderSimplified.openExternalAuthUrl(
              this.authProvider,
              authMethod,
              tokenReceiverUrl,
            );
      ...
      default:
        this.handleError(
          new Error("Invalid request type from Webview"),
          "system",
        );
    }
  }
