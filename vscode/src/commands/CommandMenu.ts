...
import { CustomCommandConfigMenuItems, menu_buttons, menu_options, menu_separators } from './utils/menu'
import type { CustomCommandsItem } from './utils'
import { vscodeDefaultCommands } from './PromptsProvider'
...
export async function showCommandMenu(
    ...
    const options = commandMenuTitle[type]
    ...
    return new Promise(resolve => {
        ...
        quickPick.onDidAccept(async () => {
            ...
            if (selected === addOption.label) {
                void commands.executeCommand('cody.action.commands.add')
                quickPick.hide()
                return
            }
