...
export async function executeTestEditCommand(
    ...
    const newTestSuitePrompt =
        'Review the shared code context to identify the testing framework and libraries in use. Then, create a new test suite with multiple new unit tests for my selected code following the same patterns, testing conventions and testing library as shown in shared context. Pay attention to the shared context to ensure your response code do not contain cases that have already been covered. Focus on generating new unit tests for uncovered cases. Response only with the full completed code for the new unit tests without any added comments, fragments or TODO. The new tests should validate expected functionality and cover edge cases for selected code. The goal is to provide me with a new test suite that I can add to the end of the existing test file. Enclose only the new test suite WITHOUT ANY import statements or modules in your response. Do not repeat tests from shared context.'
