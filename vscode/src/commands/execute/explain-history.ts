...
import { CommandsProvider } from "../services/provider";
...
async function explainHistoryCommand(
	...
	logDebug(
		"explainHistoryCommand",
		"computed history options",
		JSON.stringify(historyOptions),
	);

	let contextFiles: ContextItem[] = [];
	try {
		contextFiles = await commandsProvider.history(
			historyOptions.uri,
			historyOptions,
		);
...
export async function executeExplainHistoryCommand(
	...
	return wrapInActiveSpan("command.explain-history", async (span) => {
		...
		const sessionArgs = await explainHistoryCommand(
			span,
			commandsProvider,
			args,
		);
...
function getHistoryOptions(
	args?: Pick<CodyCommandArgs, "range">,
): HistoryOptions | FailedExplainResult {
	...
	const symbolRange = doc.getWordRangeAtPosition(
		(args?.range ?? activeEditor.selection).start,
	);
	if (!symbolRange) {
		return {
			level: "warn",
